...
if(isset($model->id)){
	Yii::app()->clientScript->registerScript('displayURL',
		"$('#display-url').click(function(){
			$.ajax({
				url: yii.scriptUrl + '/studio/displayUrl',
				type: 'POST',
				data: {id: ".$model->id ."},
				success: function(){
				$('#user-url-table').show();
	...
	Yii::app()->clientScript->registerScript('userTable',
		"$('#user-url-table').hide();"
	, CClientScript::POS_END);
}
...
<div id="main-column" <?php echo $layoutManager->columnWidthStyleAttr (1); ?>>
            ...
            //$this->renderPartial('application.components.views.@DETAILVIEW', array('model' => $model, 'modelName' => 'x2force')); ?>
...
<a class="x2-button highlight left" id="display-url" href="javascript:void(0);">
<?php echo Yii::t('app', 'Display  URL');?>
</a>
<table id="user-url-table" style="border-collapse:collapse; width:100%;"> 
	<tr>
		<th style="text-align:left;border:1px solid #ddddd;" >URL</th>
		<th style="text-align:left; border:1px solid #ddddd;" >Last Updated</th>
		<th style="text-align:right; border:1px solid #ddddd;" >Create Date</th>
	</tr>
	<tr style = "background-color: #dddddd;">
		<td style="text-align:left;"><?php 
						$id = $model->id;
						$contact = X2Force::model()->findByPk($id);
						echo $contact->url;  
						?></td>
		<td style="text-align:left;"><?php
						$id = $model->id;
						$contact = X2Force::model()->findByPk($id);
						echo date('r', $contact->lastUpdated);
						?><td>
		<td style="text-align:left;"><?php
						$id = $model->id;
						$contact = X2Force::model()->findByPk($id);
						echo date('r', $contact->createDate);
						?></td>
	</tr>
</table>
